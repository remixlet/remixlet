// Store packaging: build the Chrome target, zip it for a Chrome Web Store
// upload, and validate the exact artifact (wiki/ops/chrome-store-submission.md
// §3.3). The zip carries dist/chrome minus sourcemaps and build-id.txt — the
// bundles ship unminified, so review readability never needed the maps — with
// fixed entry timestamps and sorted paths, so one built tree always zips to the
// same bytes. Validation re-reads the zip it just wrote: central directory,
// per-entry CRCs, manifest contracts, and the files the manifest points at.
...
console.log("== build");
execFileSync(process.execPath, ["build.mjs", "--target=chrome"], { cwd: extensionRoot, stdio: "inherit" });
...
const zipPath = path.join(extensionRoot, "dist", `remixlet-chrome-${version}.zip`);
...
console.log(`${zipPath}`);
console.log(
  `DONE — ${names.size} files, ${mb(totalUncompressed)} unpacked → ${mb(zip.length)} zipped, sha256 ${createHash("sha256").update(zip).digest("hex")}`,
);
